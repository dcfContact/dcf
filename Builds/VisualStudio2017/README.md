...
We do not recommend Windows for dcf production use at this time. Currently,
...
modification, build dcf under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build dcf. The **Visual
...
* NOTE: Since dcf links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. dcf does not use
...
The dcf application is linked statically to the standard runtimes and
...
process and clone `git@github.com:ripple/dcf.git`. Otherwise follow this
...
above. Navigate to the directory where you want to clone dcf (git bash uses
...
listed on [dcf's GitHub page](https://github.com/ripple/dcf/releases). 
...
5. Select either the `dcf.exe` (unity) or `dcf_classic.exe` (non-unity)
   ...
   dcf repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\dcf.exe
```
or
```
.\build\x64-Debug\Debug\dcf.exe
```
...
execute the following commands within your `dcf` cloned repository:
...
Navigate to the `build\cmake` folder created above and select the `dcf.sln`
file. You can then choose whether to build the `Debug` or `Release` solution
configuration.

The executable will be in 
```
.\build\cmake\Release\dcf.exe
```
or
```
.\build\cmake\Debug\dcf.exe
```
These paths are relative to your cloned git repository.

# Unity/No-Unity Builds

The dcf build system defaults to using [unity source files](http://onqtam.com/programming/2018-07-07-unity-builds/)
...
`dcf` builds a set of unit tests into the server executable. To run these
unit tests after building, pass the `--unittest` option to the compiled
`dcf` executable. The executable will exit with summary info after running
the unit tests.
