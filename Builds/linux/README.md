...
This document focuses on building dcf for development purposes under recent
Ubuntu linux distributions. To build dcf for Redhat, Fedora or Centos
builds, including docker based builds for those distributions, please consult
the [dcf-package-builder](https://github.com/ripple/dcf-package-builder)
repository. 
...
At this time, dcf only supports protobuf version 2. Using version 3 of 
...
listed on [GitHub](https://github.com/ripple/dcf/releases). 
...
The dcf cmake build supports an installation target that will install
dcf as well as a support library that can be used to sign transactions. In
...
dcf project that you can link to in your own project's CMakeLists.txt, e.g.:
...
First, add the dcf repo as a submodule to your project repo:

```
git submodule add -b master https://github.com/ripple/dcf.git vendor/dcf
```

change the `vendor/dcf` path as desired for your repo layout. Furthermore,
change the branch name if you want to track a different dcf branch, such
as `develop`.
   
Second, to bring this submodule into your project, just add the dcf subdirectory:

```
add_subdirectory (vendor/dcf)
```
    
##### Option 2: installed dcf + find_package

First, follow the "Optional Installation" instructions above to
build and install the desired version of dcf.
