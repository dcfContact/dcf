...
set -ex
docker login -u dcf \
    -p ${ARTIFACTORY_DEPLOY_KEY_dcf} "${ARTIFACTORY_HUB}"
# this gives us dcf_version :
...
# tag/push two labels...one using the current dcf version and one just using "latest"
for label in ${dcf_version} latest ; do
    docker tag \
