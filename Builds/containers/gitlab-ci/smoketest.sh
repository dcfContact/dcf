...
if [ "$use_private" -gt 0 ] ; then
    REPO_ROOT="https://dcf:${ARTIFACTORY_DEPLOY_KEY_dcf}@${ARTIFACTORY_HOST}/artifactory"
else
...
if [ "${pkgtype}" = "dpkg" ] ; then
    ...
    if [ "${install_from}" = "repo" ] ; then
        ...
        # apt-cache policy dcf
        apt-get -y install dcf=${dpkg_full_version}
    elif [ "${install_from}" = "local" ] ; then
        # cached pkg install
        updateWithRetry
        apt-get -y install libprotobuf-dev libssl-dev
        rm -f build/dpkg/packages/dcf-dbgsym*.*
...
INSTALLED=$(/opt/ripple/bin/dcf --version | awk '{print $NF}')
if [ "${dcf_version}" != "${INSTALLED}" ] ; then
    echo "INSTALLED version ${INSTALLED} does not match ${dcf_version}"
    exit 1
fi
# run unit tests
/opt/ripple/bin/dcf --unittest --unittest-jobs $(nproc)
