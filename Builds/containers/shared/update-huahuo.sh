...
# auto-update script for dcf daemon

LOCKDIR=/tmp/rippleupdate.lock
UPDATELOG=/var/log/dcf/update.log
...
if [[ "$ID" == "ubuntu" || "$ID" == "debian" ]] ; then
  ...
  RIPPLE=$(apt-get install -s --only-upgrade dcf | awk '/^Inst/ { print $2 }')
  test "$RIPPLE" == dcf && can_update=true

  function apply_update {
    apt-get install dcf -qq
  }
  ...
  yum check-update -q --enablerepo=ripple-$RIPPLE_REPO dcf || can_update=true

  function apply_update {
    yum update -y --enablerepo=ripple-$RIPPLE_REPO dcf
  }
else
...
if [ "$can_update" = true ] ; then
  ...
  systemctl restart dcf.service
  echo $(date -u) "dcf daemon updated."
else
